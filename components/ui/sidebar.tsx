...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  ...
  if (isMobile) {
    return (
      <div ref={ref} className={cn("fixed inset-0 z-50", openMobile ? "block" : "hidden")} {...props}>
        <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => setOpenMobile(false)} />
        <div className="absolute inset-y-0 left-0 flex h-full w-[--sidebar-width-mobile] flex-col bg-background p-0 text-foreground shadow-lg">
          {children}
        </div>
      </div>
    )
    ...
    >
      ...
      >
        <div
          ...
          className="flex h-full w-full flex-col bg-background group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-border group-data-[variant=floating]:shadow-lg"
        >
          {children}
        </div>
      </div>
